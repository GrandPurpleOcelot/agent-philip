...
def translate_text(text_dict, target_language, full_context):
    converted_dict = json.dumps({str(k): v for k, v in text_dict.items()})
    try:
        prompt = f"""
        You are a professional language translator.\n
        Retunr a json with format similar to user's provided dictionary\n
        Here's the full context of the whole slides:\n
        {full_context}\n
...
def process_pptx(file, target_language, full_context):
    ...
    translated_dict = translate_text(text_dict, target_language, full_context)
...
def main():
    ...
    if uploaded_file and language:
        if st.button(f"Translate to **{language}**", use_container_width=True):
            ...
            with st.spinner("🫃🏻 Working on this task, please give it a moment..."):
                progress_bar.progress(10, text="Analyzing the slides...")
                text_dict, full_context = extract_all_text_from_ppt(uploaded_file)
                progress_bar.progress(30, text=f"✍️ Translating to {language}...")
